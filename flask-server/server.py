...
CORS(app, resources={
    r"/*": {
        "origins": ["http://localhost:3000", "http://localhost:3001", "http://localhost:5001", "https://vyun8699.github.io", "https://vyun8699.github.io/NoCodeEDA", "https://vyun8699.github.io/NoCodeEDA/"],
        "methods": ["GET", "POST"],
...
def create_visualization():
    ...
    try:
        ...
        if not processor.current_df is not None:
            return jsonify({'success': False, 'error': 'No data loaded'}), 400
            
        visualization = processor.create_visualization(parameters, viz_type)
        if visualization:
            # Test if the visualization is valid base64
            try:
                base64.b64decode(visualization)
                logger.info("Successfully validated base64 visualization data")
                return jsonify({
                    'success': True,
                    'visualization': visualization
                })
            except Exception as e:
                logger.error(f"Invalid base64 data: {str(e)}", exc_info=True)
                return jsonify({
                    'success': False,
                    'error': 'Invalid visualization data format'
                }), 500
        else:
            logger.error("Visualization creation failed but no error was raised")
            return jsonify({
                'success': False,
        ...
        logger.error(f"Validation error in create_visualization: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 400
    except Exception as e:
        logger.error(f"Error in create_visualization: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route("/head", methods=['GET'])
def get_head():
    """Get first 20 rows of the dataset"""
    try:
        if processor.current_df is None:
            return jsonify({'success': False, 'error': 'No data loaded'}), 400
            
        # Convert DataFrame to records and handle null values
        head_data = processor.current_df.head(20).fillna('').to_dict('records')
        ...
        logger.error(f"Error in get_head: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
...
def save_data():
    ...
    try:
        ...
        logger.error(f"Error in save_data: {str(e)}", exc_info=True)
        return jsonify({'success': False, 'error': str(e)}), 500
