...
export const VISUALIZATION_TYPES = {
  ...
  MIXED: [  // For categorical x-axis with numeric y-axis
    { id: 'bar', name: 'Bar Chart' },
    { id: 'line', name: 'Line Plot (Mean per Category)' }
  ]
};
...
export const getVisualizationsForType = (dataType, paramCount = 1) => {
  ...
  if (paramCount === 1) {
    ...
    if (dataType === 'categorical') {
      types = [...VISUALIZATION_TYPES.MIXED];
    } else {
...
export const fetchAPI = async (endpoint, options = {}) => {
  try {
    console.log(`Making ${options.method || 'GET'} request to ${endpoint}`, options.body);
